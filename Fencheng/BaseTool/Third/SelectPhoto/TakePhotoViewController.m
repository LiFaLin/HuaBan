...
- (void)createCamerDistrict{
    ...
    self.previewLayer.frame = CGRectMake(0, 35, APP_SCREEN_WIDTH, APP_SCREEN_HEIGHT - self.bottomView.frame.size.height - 35);
...
//    [self focusAtPoint:CGPointMake(179, 314)];
}
...
- (void)setupFocusPointManual{
    ...
    CGPoint point = CGPointMake(APP_SCREEN_WIDTH / 2.0, 35 + self.previewLayer.bounds.size.height / 2.0);
    [self focusAtPoint:point];
}
...
- (IBAction)takePhotos:(UIButton *)sender {
    ...
    [self.imageOutput captureStillImageAsynchronouslyFromConnection:conntion completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
        ...
        UIImage *jianqie= [UIImage thumbnailWithImage:image size:CGSizeMake(APP_SCREEN_WIDTH+250, APP_SCREEN_HEIGHT+250)];
        [self.session stopRunning];
        
        [self.view addSubview:self.clipView];
        self.clipView.image = jianqie;
        ...
        self.clipView.sureUseBlock = ^(UIImage *clipImage) {
            ...
            [self saveImageToPhotoAlbum:clipImage];
            [self dismissViewControllerAnimated:YES completion:^{
