...
//@property (nonatomic, strong) lflSlider *slider;
...
- (void)layoutSubviews {
    ...
    if (!self.isOriginalFrame) {
        ...
        self.setButton.frame=CGRectMake((self.playerOriginalFrame.size.width - playBtnSideLength) / 2, 20, 60,30);
...
//
//        self.setButton.frame=CGRectMake((self.keyWindow.bounds.size.height - playBtnSideLength) / 2, 20,60,30);
//    }];
//
...
- (void)show {
    [UIView animateWithDuration:barAnimateSpeed animations:^{
        ...
        self.setButton.layer.opacity=opacity;
...
- (void)hiden {
    ...
    [UIView animateWithDuration:barAnimateSpeed animations:^{
        ...
        self.setButton.layer.opacity=0.0f;
    } completion:^(BOOL finished){
        if (finished) {
            self.barHiden = !self.barHiden;
        }
...
//- (void)sliderValueChange:(lflSlider *)slider {
...
//        
//        lflSlider *slider = [[lflSlider alloc] init];
//        slider.value = 0.0f;
...
//        self.slider = slider;
//        __weak typeof(self) weakSelf = self;
//        slider.valueChangeBlock = ^(lflSlider *slider){
//            [weakSelf sliderValueChange:slider];
//        };
//        slider.finishChangeBlock = ^(lflSlider *slider){
//            [weakSelf finishChange];
//        };
//        slider.draggingSliderBlock = ^(lflSlider *slider){
//            [weakSelf dragSlider];
//        };
...
//        [_setButton setBackgroundColor:[UIColor orangeColor]];
        ...
        _setButton.layer.opacity=0.0f;
        [_setButton setTitleColor:[UIColor blueColor] forState:UIControlStateSelected];
